...
function mockFetchSequence(responses: Array<{ ok:boolean; json:any; etag?:string; status?:number }>) {
  let i = 0;
  return vi.fn(async (url: string, init?: RequestInit) => {
    ...
    return {
      ...
      json: async () => r.json,
      headers: { get: (k:string)=> (k==='ETag' ? r.etag ?? null : null) }
    } as any;
  });
}
...
describe('apiChannel', () => {
  it('reads value', async () => {
    const fetchImpl = mockFetchSequence([{ ok: true, json: { value: { a: 1 } }, etag: 'v1' }]);
  ...
  it('queues on failure and flushes later', async () => {
    ...
    const ch = apiChannel('k', { baseUrl: 'http://x', fetchImpl, storagePrefix:'t' });
    ...
    // manual flush call via internal is not exposed; emulate by calling write again to trigger second fetch
