...
export function storageEncryptedChannel<T>(key: string, opts: EncryptedStorageOptions): Channel<T> {
  const {
    passphrase,
    saltKey = `syncr:enc:salt`,
    keyIter = 120000,
    ivBytes = 12,
    storage = (isBrowser() ? window.localStorage : undefined) as Storage | undefined
  } = opts;
  ...
  const subscribe = (cb: (v:T|undefined)=>void) => {
    ...
    const handler = (e: StorageEvent) => {
      if (e.key === key) {
        read().then(v => cb(v)).catch(()=>{});
      }
    };
    if (isBrowser()) window.addEventListener('storage', handler);
    return () => { if (isBrowser()) window.removeEventListener('storage', handler); };
  };

  return { id: 'storage-encrypted', priority: 0, read, write, subscribe };
}
